...
function WoWJapanizer_Spell:Get(id)
...
function WoWJapanizer_Spell:GetNextRank(id)
    if type(id) == 'number' then
        id = tostring(id)
    end
...
function WoWJapanizer_Spell:GetGlyphByName(name)
...
function WoWJapanizer_Spell:kFormatter(num)
	num = tonumber(num)
	if num > 9999 then
...
function WoWJapanizer_Spell:convertMacro(ja, en)
	local n = {}
	local j = 1
	...
	for i in string.gmatch(en, "[%d%,%.]+") do
		...
		if i ~= '' then
			...
			-- n['N' .. tostring(j)] = i
			n['N' .. tostring(j)] = self:kFormatter(i)
			
			j = j + 1
		end
	end

	return string.gsub(ja, "%$(N%d+)", n)
end

function WoWJapanizer_Spell:GetTooltipText(id, tooltip)
    ...
    if string.match(spell.text, "$N%d+") then
		...
		if target2 then
             if target2:GetText() == 'Next rank:' then
                 ...
                 if spellnext then
                     spell.next = self:convertMacro(spellnext.text, target:GetText())
                 end
        ...
        spell.text = self:convertMacro(spell.text, target:GetText())
    end

    return spell
end

function WoWJapanizer_Spell:GetBuff(id)
    if type(id) == 'number' then
        id = tostring(id)
    end
...
function WoWJapanizer_Spell:GetBuffTooltipText(id, tooltip)
